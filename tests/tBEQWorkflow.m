classdef tBEQWorkflow < matlab.unittest.TestCase
    properties
        Folder
        Data 
    end
    ...
    methods (Test)
        function tnoLoopNoEval(tc)
            ms = NowcastConfig('Example1', 'BEQ', do_loop=0, do_eval = 0);
        ...
        function tLoop1Eval(tc)
            ms = NowcastConfig('Example1', 'BEQ', do_loop=1, do_eval = 0); % auto set to eval = 1
            er = nowcast.estimate(ms, excel_datafile=tc.Data, outputfolder=fullfile(tc.Folder, 'output'));
        end

        function tnoLoop2Eval(tc)
            ms = NowcastConfig('Example1', 'BEQ', do_loop=2, do_eval = 1);
            er = nowcast.estimate(ms, excel_datafile=tc.Data, outputfolder=fullfile(tc.Folder, 'output'));
        end
    end
end